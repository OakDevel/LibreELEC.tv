...
class fd628Addon():
	...
	def run(self):
		with fd628dev.fd628Dev() as fd628:
			self._fd628 = fd628
			self.__createStates()
			...
			while not self._monitor.abortRequested():
				...
				for state in self._states:
					state.update()
					if (state.hasChanged() or fd628states.enableDebug):
						if (state.getValue()):
	...
	def onSettingsChanged(self):
		...
		if (self._fd628 != None):
			kodiLog('Enter self._fd628 != None')
	...
	def __createStates(self):
		...
		'musicsettings', 'appearancesettings', 'picturessettings', 'weathersettings', \
		...
		if (self._settings.isStorageIndicator()):
			for state in states:
				if (state.getLedName() == self._settings.getStorageIndicatorIcon()):
					states.remove(state)
					break
			states.append(fd628states.fd628ExtStorageCount(self._settings.getStorageIndicatorIcon(), ['/sys/block/sda/stat', '/sys/block/mmcblk1/stat'], 'rw'))
			kodiLog(str([str(state) for state in states]))
		self._states = states
